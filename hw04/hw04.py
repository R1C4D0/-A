def shuffle(s):
    ...
    for i in range(len(s)//2):
        ret_seq.append(s[i])
        ret_seq.insert(i*2+1,s[i+len(s)//2])
    return ret_seq
...
def deep_map(f, s):
    ...
    """
    "*** YOUR CODE HERE ***"
    # use recursion to iterate through the nested list
...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    # check if the mobile is balanced by checking if the torque of the left arm is equal to the torque of the right arm
    if is_planet(m):
        return True
    else:
        assert is_mobile(m), "must get total mass of a mobile or a planet"
...
def berry_finder(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    # use recursion to iterate through the tree
    if label(t) == 'berry':
        return True
    for b in branches(t):
        if berry_finder(b):
