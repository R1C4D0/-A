...
def floor_div(args):
    ...
    """
    "*** YOUR CODE HERE ***"
    assert isinstance(args, Pair), "Arguments must be a Pair"
    assert len(args) >= 2, "At least 2 arguments are required"
    if len(args) == 2:
        return args.first // args.rest.first
    else:
        args = Pair(args.first // args.rest.first, args.rest.rest)
...
def eval_and(expressions):
    ...
    """
    "*** YOUR CODE HERE ***"
    # assumes conditional operators (e.g. <, >, =, etc) have already been implemented, 
    # but you do not have to worry about them for this question.
    if expressions is nil:# empty pair
        return True
    if expressions.first is False:# exactly Scheme's #f
        return False
    if expressions.rest is nil:# only one element, return that element
        return calc_eval(expressions.first)
    return eval_and(expressions.rest)# recursive call
