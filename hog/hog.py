...
def roll_dice(num_rolls, dice=six_sided):
    ...
    num_rolls:  The number of dice rolls that will be made. NOTICE: exactly NUM_ROLLS > 0, even if the dice is 1.
...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    plarer_one = player_score % 10
    opponent_ten = (opponent_score % 100) // 10 if opponent_score >= 10 else 0
    result = 1 if plarer_one == opponent_ten else 3 * abs(plarer_one - opponent_ten)
    return result
    # END PROBLEM 2
...
def sus_points(score):
    ...
    if 3 == num_factor or 4 == num_factor:
        for i in range(score + 1, 2 * score, 1):
            if is_prime(i):
                return i
    else :
        return score            
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    num_factor = num_factors(player_score)
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    "每次都要使用新的averaged_roll_dice，否则会出现错误，如times_called不能被make_test_dice的面数整除"
    max ,max_socring_num = 0, 1
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    if threshold <= sus_update(0, score, opponent_score) - score:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
