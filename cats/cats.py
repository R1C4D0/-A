...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    if not typed_words and not source_words:# both are empty
        return 100.0
    elif not typed_words or not source_words:# only one is empty
        return 0.0
    correct_count = 0
...
def furry_fixes(typed, source, limit):
    ...
    if not typed or not source:
        return len(typed) + len(source) # if either is empty, return the sum of their lengths, length of empty string is 0
    if typed[0] != source[0]:
        return 1 + furry_fixes(typed[1:], source[1:], limit - 1) # if the first characters are different, add 1 and recurse
    else :
...
def minimum_mewtations(typed, source, limit):
    ...
    3
    """
    beyond_limit = limit + 1
    # Base cases should go here, you may add more base cases as needed.
    if not typed or not source: 
        return len(typed) + len(source) # 0 if both are empty, else the length of the non-empty string
...
def time_per_word(words, timestamps_per_player):
    ...
    #同时遍历i和i+1
    for timestamps_one_player in timestamps_per_player:
        times_one_player = []
        for i in range(len(timestamps_one_player) - 1):
            times_one_player.append(timestamps_one_player[i + 1] - timestamps_one_player[i])
        times.append(times_one_player)
    # END PROBLEM 9
